...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden = last_hidden.reshape(len(enc_hiddens), 2*self.hidden_size)
        last_cell = last_cell.reshape(len(enc_hiddens), 2 * self.hidden_size)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        U_t = torch.cat((dec_hidden, torch.squeeze(a_t)),1)
